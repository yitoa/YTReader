...
package com.yt.reader.receiver;
...
public class ScanSdcardReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Intent.ACTION_MEDIA_SCANNER_STARTED.equals(action)) {
            ...
            ad.cancel();
            if (count >= 0) {
                Toast.makeText(context, count + " was added", Toast.LENGTH_LONG).show();
            } else {
                Toast.makeText(context, Math.abs(count) + " was cutted", Toast.LENGTH_LONG).show();
            }
        }
    }
}
