...
public abstract class PageView extends ViewGroup {
    ...
    private final PDFActivity context;
    ...
    public PageView(PDFActivity c, Point parentSize) {
        super(c);
        context = c;
        mParentSize = parentSize;
        setBackgroundColor(BACKGROUND_COLOR);
        mUsingHardwareAcceleration = Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD;
    }
    ...
    public void setPage(int page, PointF size) {
        ...
        mEntireBm =context.application.getCurPageBitmap();
        // Render the page in the background
        mDrawEntire = new AsyncTask<Void, Void, LinkInfo[]>() {
            protected LinkInfo[] doInBackground(Void... v) {
                drawPage(mEntireBm,context.screenWidth, context.screenHeight, 0, 0,context.screenWidth, context.screenHeight);
                return getLinkInfo();
            }
        ...
        if (mSearchView == null) {
            mSearchView = new View(context) {
                @Override
                protected void onDraw(Canvas canvas) {
                    super.onDraw(canvas);
                    float scale = mSourceScale * (float) getWidth() / (float)context.screenWidth;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mBusyIndicator != null) {
            ...
            mBusyIndicator.measure(View.MeasureSpec.AT_MOST | limit, View.MeasureSpec.AT_MOST
                    | limit);
        }
    }
    ...
    public void addHq() {
        ...
        // for an hq patch
        if (viewArea.width() !=context.screenWidth || viewArea.height() != context.screenHeight) {
            ...
            Bitmap bm = context.application.getCurPageBitmap();
            Log.v("bitmap",patchArea.width()+" "+patchArea.height());
//            try {
//                bm = Bitmap.createBitmap(patchArea.width(), patchArea.height(),
//                        Bitmap.Config.ARGB_8888);
//                WeakReference<Bitmap> bitmapcache = new WeakReference<Bitmap>(bm);// 添加一个软引用要是内存溢出可以释放内存
//                bm = null;
//                bm = bitmapcache.get();
//            } catch (Error e) {
//                CrashHandler.uncaughtError(context, e);
//            }

            mDrawPatch = new AsyncTask<PatchInfo, Void, PatchInfo>() {
                protected PatchInfo doInBackground(PatchInfo... v) {
                    drawPage(v[0].bm, v[0].patchViewSize.x, v[0].patchViewSize.y,
                            v[0].patchArea.left, v[0].patchArea.top, v[0].patchArea.width(),
