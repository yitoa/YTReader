...
static int
fillInSearchResults(JNIEnv * env, jclass searchClass, jmethodID ctor, jobjectArray arr, int i4pos, int i4page, const char *psearchresult, float left, float top, float right, float bottom)
{
	...
	jobject search = (*env)->NewObject(env, searchClass, ctor, i4page, title, left, top, right, bottom, -1, -1, -1, -1);
	if (search == NULL) 
	{
	...
	PDFLOGI("[mupdf.c] fillInSearchResults, ARRAY[%d]: Page=%d, left=%7.2f, top=%7.2f, right=%7.2f, bottom=%7.2f, Title=%s \n", i4pos, i4page, left, top, right, bottom, psearchresult);
	i4pos = i4pos + 1;
	return i4pos;
}
		...
		for (i4pagenumber = 0; i4pagenumber < i4TotalPageNum; i4pagenumber++)
		{
			...
			for (pos = 0; pos < len; pos++)
			{
				...
				if(n > 0)
				{
					...
					//PDFLOGI("[mupdf.c] [X0,Y0] => left=%7.2f, top=%7.2f \n", left, top);
					//PDFLOGI("[mupdf.c] [X1,Y1] => right=%7.2f, bottom=%7.2f \n", right, bottom);		
					
					for (i4num = pos; i4num < len; i4num++)
					{
						int i4Result = pdf_unicode_to_utf8(charat(text, i4num), utfchar);
						//PDFLOGI("[mupdf.c] i4Result = %d, charat(text, i4num) = %d, utfchar = %s, strlen(utfchar) = %d \n", i4Result, charat(text, i4num), utfchar, strlen(utfchar));
					...
					{
						i4arraypos = fillInSearchResults(env, searchClass, ctor, arr, i4arraypos, i4pagenumber, searchresult, left, top, right, bottom);
						if(i4arraypos == -1)
						{
