...
pdf_load_version(pdf_document *xref)
{
	...
	PDFLOGI("[pdf_xref.c] pdf_load_version xref->version is %d, => %d.%d \n", xref->version, (xref->version)/10, (xref->version)%10);
}
...
pdf_read_start_xref(pdf_document *xref)
{
	...
	PDFLOGI("[pdf_xref.c] pdf_read_start_xref xref->file_size is %d \n", xref->file_size);
...
#if JUSTTESTPDF // for test
	PDFLOGI("[pdf_xref.c] pdf_read_start_xref t = %d \n", t);
	PDFLOGI("[pdf_xref.c] pdf_read_start_xref n = %d \n", n);
#endif
	...
	for (i = n - 9; i >= 0; i--)
	{
		if (memcmp(buf + i, "startxref", 9) == 0)
		{
			...
			PDFLOGI("[pdf_xref.c] pdf_read_start_xref xref->startxref is %d \n", xref->startxref);
			return;
		}
	}
...
{
	PDFLOGI("[pdf_xref.c] pdf_read_old_trailer start \n");
...
{
	PDFLOGI("[pdf_xref.c] pdf_read_new_trailer start \n");
...
pdf_read_trailer(pdf_document *xref, pdf_lexbuf *buf)
{
	...
	fz_try(xref->ctx)
	{
		c = fz_peek_byte(xref->file);
#if JUSTTESTPDF //for test
		PDFLOGI("[pdf_xref.c] pdf_read_trailer c is '%c', %d \n", c, c);
#endif
		if (c == 'x')
...
{
#if JUSTTESTPDF // for test
	PDFLOGI("[pdf_xref.c] pdf_resize_xref newlen = %d \n", newlen);
#endif
	int i;
...
{
	PDFLOGI("[pdf_xref.c] pdf_load_xref start \n");
...
#if JUSTTESTPDF // for test
	int i4size = pdf_to_int(size);
	PDFLOGI("[pdf_xref.c] pdf_load_xref trailer's i4size = %d \n", i4size);
#endif
...
#if JUSTTESTPDF //for test
	PDFLOGI("[pdf_xref.c] pdf_load_xref xref's length = %d \n", xref->len);
	PDFLOGI("------------------------------------------------- xref start \n");
	PDFLOGI("   ID	file_offset    generation   type       on-disk_stream 	\n");
	for (i = 0; i < xref->len; i++)
	{
		PDFLOGI("[%4d]  %010d,    %05d,     %c,     %d \n", 
			i, xref->table[i].ofs, xref->table[i].gen, xref->table[i].type, xref->table[i].stm_ofs);
	}
	PDFLOGI("------------------------------------------------- xref end \n");
...
{
	PDFLOGI("[pdf_xref.c] pdf_open_document_with_stream start\n");
...
{
	PDFLOGI("[pdf_xref.c] pdf_open_document \n");
	PDFLOGI("[pdf_xref.c] pdf_open_document, filename is %s \n", filename);
...
{
	PDFLOGI("[pdf_xref.c] pdf_load_outline_shim start\n");
...
{
	//PDFLOGI("[pdf_xref.c] pdf_load_page_shim start\n");
