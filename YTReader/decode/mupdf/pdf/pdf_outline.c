...
{
	PDFLOGI("[pdf_outline.c] pdf_load_outline_imp start \n");
	...
	fz_try(ctx)
	{
		...
		while (dict && pdf_is_dict(dict))
		{
			...
			*prev = node;
			...
			if (obj)
			{
				//char *testoutline = pdf_to_str_buf(obj);
				//PDFLOGI("[pdf_outline.c] pdf_load_outline_imp, unicode_outline = %s \n", testoutline);
				//char *rederoutline = pdf_to_utf8_from_string(ctx, testoutline);
				//PDFLOGI("[pdf_outline.c] pdf_load_outline_imp, utf8_outline = %s \n", rederoutline);
					
				node->title = pdf_to_utf8(ctx, obj);
				PDFLOGI("[pdf_outline.c] pdf_load_outline_imp, outline title = %s \n", node->title);
			}
