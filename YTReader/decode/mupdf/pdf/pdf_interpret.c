...
#if SUPPORT_BOOKMARK
extern char g_chBookmarkStr[128];
#endif
...
pdf_show_char(pdf_csi *csi, int cid)
{
	...
	if (render_direct)
	{
		PDFLOGI("[pdf_interpret.c] pdf_show_char, direct render \n");	
	...
	else
	{
		...
		int i4Length = strlen(g_chBookmarkStr);
		//PDFLOGI("[pdf_interpret.c] pdf_show_char, g_chBookmarkStr's length = %d \n", i4Length);	
		// It maybe show 100 charactor in reader screen, less than 128 charactor.
		if(100 >= i4Length)
		{	
			unsigned char utfchar[128] = {0};
			int i4Result = pdf_unicode_to_utf8((unsigned long)(ucsbuf[0]), utfchar);
			//PDFLOGI("[pdf_interpret.c] pdf_show_char, unicode convert utf8, i4Result = %d, ucsbuf[0]= %d, utfchar = %s \n", i4Result, ucsbuf[0], utfchar);

			int i4SrcLength = 128 - i4Length;
			strncat(g_chBookmarkStr, utfchar, i4SrcLength);
			g_chBookmarkStr[127] = '\0';

			//PDFLOGI("[pdf_interpret.c] pdf_show_char, g_chBookmarkStr = %s \n", g_chBookmarkStr);
		}
...
{
	//PDFLOGI("[pdf_interpret.c] pdf_show_space start \n");	
...
{
	//PDFLOGI("[pdf_interpret.c] pdf_show_string start \n");	
...
{
	//PDFLOGI("[pdf_interpret.c] pdf_show_text start \n");	
	//PDFLOGI("[pdf_interpret.c] pdf_show_text, pdf_is_array(text) = %d, pdf_is_string(text) = %d \n", pdf_is_array(text), pdf_is_string(text));
	...
	{
		//PDFLOGI("[pdf_interpret.c] pdf_show_text pdf_array_len(text) = %d \n", pdf_array_len(text));
		...
		for (i = 0; i < n; i++)
		{
			...
			{
				//PDFLOGI("[pdf_interpret.c] pdf_show_text, pdf_show_string\n");
				pdf_show_string(csi, (unsigned char *)pdf_to_str_buf(item), pdf_to_str_len(item));
			}
			else
			{
				//PDFLOGI("[pdf_interpret.c] pdf_show_text, pdf_show_space\n");
...
{
	//PDFLOGI("[pdf_interpret.c] pdf_run_Tj, csi->string_len = %d \n", csi->string_len);
...
pdf_run_stream(pdf_csi *csi, pdf_obj *rdb, fz_stream *file, pdf_lexbuf *buf)
{
	...
	while (1)
	{
		...
		//PDFLOGI("[pdf_interpret.c] pdf_run_stream, tok = %d \n", tok);
