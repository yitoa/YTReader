...
pdf_load_image_stream(pdf_document *xref, int num, int gen, pdf_image_params *params)
{
	...
	len = pdf_to_int(pdf_dict_gets(dict, "Length"));
	//PDFLOGI("[pdf_stream.c] pdf_load_image_stream, content length (original) = %d \n", len);
	obj = pdf_dict_gets(dict, "Filter");
	len = pdf_guess_filter_length(len, pdf_to_name(obj));
	//PDFLOGI("[pdf_stream.c] pdf_load_image_stream, content length (after 'Filter') = %d \n", len);
	n = pdf_array_len(obj);
	for (i = 0; i < n; i++)
	...
	//PDFLOGI("[pdf_stream.c] pdf_load_image_stream, content length (after array 'Filter') = %d \n", len);
