...
pdf_load_page_contents(pdf_document *xref, pdf_obj *obj)
{
	...
	if (pdf_is_array(obj))
	{
		//PDFLOGI("[pdf_page.c] pdf_load_page_contents, PDF is array \n");
	...
	{
		//PDFLOGI("[pdf_page.c] pdf_load_page_contents, PDF is stream \n");
		//PDFLOGI("[pdf_page.c] pdf_load_page_contents, num = %d, gen = %d \n", pdf_to_num(obj), pdf_to_gen(obj));
...
{
	//PDFLOGI("[pdf_page.c] pdf_load_page start\n");
	fz_context *ctx = xref->ctx;
	pdf_page *page;
	pdf_annot *annot;
	...
	fz_try(ctx)
	{
		...
		// just test it, remove later
		//int i4len = page->contents->len;
		//int i4cap = page->contents->cap;
		//int i4refs = page->contents->refs;
		//PDFLOGI("[pdf_page.c] i4len = %d, i4cap = %d, i4refs = %d \n", i4len, i4cap, i4refs);

		//char pagecontent[257] = {0};
		//strncpy(pagecontent, (char *)(page->contents->data), 256);
		//PDFLOGI("[pdf_page.c] pagecontent = %s \n", pagecontent);
		// end test
