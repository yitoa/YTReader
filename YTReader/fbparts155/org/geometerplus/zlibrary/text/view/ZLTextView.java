...
public abstract class ZLTextView extends ZLTextViewBase {
	...
	//行信息，记录每一行的ZLTextLineInfo，即每一行的起始指针和结束指针等信息（paragraph、elementIndex、charIndex）
	...
	private synchronized void gotoMark(ZLTextMark mark) {
		...
		if (doRepaint) {
			...
			//Application.getViewWidget().reset();
			Application.getViewWidget().postInvalidate();
		}
	}
	...
	private ZLTextLineInfo processTextLine(
		...
		do {
			ZLTextElement element = paragraphCursor.getElement(currentElementIndex);
			newWidth += getElementWidth(element, currentCharIndex);			//计算字符串宽度（像素）
			newHeight = Math.max(newHeight, getElementHeight(element));		//计算字符串高度，包含行距（像素）
	...
	private void prepareTextLine(ZLTextPage page, ZLTextLineInfo info, int y) {
		y = Math.min(y + info.Height, getBottomLine());	//y坐标
		...
		int x = getLeftMargin() + info.LeftIndent;	//x起始坐标
