...
public class ZLTextPlainModel implements ZLTextModel, ZLTextStyleEntry.Feature {
	...
	final class EntryIteratorImpl implements ZLTextParagraph.EntryIterator {
		...
		public void next() {
			...
			case ZLTextParagraph.Entry.TEXT:
				myTextLength = (int) data[dataOffset++]
			...
			case ZLTextParagraph.Entry.STYLE: {
				...
				if (ZLTextStyleEntry.isFeatureSupported(mask, FONT_FAMILY)) {
					...
					entry.setFontFamily(new String(data, dataOffset,
							familyLength));
					dataOffset += familyLength;
				}
				if (ZLTextStyleEntry.isFeatureSupported(mask,
						FONT_STYLE_MODIFIER)) {
					final short value = (short) data[dataOffset++];
					entry.setFontModifiers((byte) (value & 0xFF),
							(byte) ((value >> 8) & 0xFF));
				}
	...
	protected ZLTextPlainModel(String id, String language, int[] entryIndices,
			int[] entryOffsets, int[] paragraphLenghts, int[] textSizes,
			byte[] paragraphKinds, CharStorage storage,
	...
	public final ZLTextMark getLastMark() {
		return ((myMarks == null) || myMarks.isEmpty()) ? null : myMarks
				.get(myMarks.size() - 1);
	}
	...
	public final int search(final String text, int startIndex, int endIndex,
			boolean ignoreCase) {
		int count = 0;
			...
			while (true) {
				...
				while (it.hasNext()) {
					...
					if (it.getType() == ZLTextParagraph.Entry.TEXT) {
						...
						int textOffset = it.getTextOffset();//textOffset为char数组中的偏移位置
						int textLength = it.getTextLength();//textLength为段落字符数
						for (int pos = ZLSearchUtil.find(textData, textOffset,
								textLength, pattern); pos != -1; pos = ZLSearchUtil
								.find(textData, textOffset, textLength,
										pattern, pos + 1)) {
							String para = null;
							int start_index = textOffset + pos;
							int end_index = textLength - pos > 40 	//剩余字符
									? start_index + 40
									: start_index + textLength - pos;
							if (myMarks.size() < 500) {				//结果集限制在500个
								para = String.valueOf(textData, start_index,
										end_index - start_index);
								ZLTextMark textMark = new ZLTextMark(index, offset
										+ pos, pattern.getLength());
								textMark.setText(para);
								myMarks.add(textMark);
							} else {
								break;
							}
	...
	public final void getLocations() {
		if (null == myMarks)
			return;
		int size = myMarks.size();
		try {
			System.out.println("fb搜索到的数量：" + size);
			for (int i = 0; i < size; i++) {
				int ParagraphIndex = myMarks.get(i).ParagraphIndex;
				Long location = mergeLocation(ParagraphIndex, 0, 0);
				String text = myMarks.get(i).myText;
				TextSearch.searchResultLocation.add(location);
				TextSearch.searchResultDescription.add(text);
			}
		} catch (Exception e) {
			System.out.println("fb搜索错误");
			e.printStackTrace();
		}
	}
	 ...
	 * @return long（64位）
	 */
	public long mergeLocation(long myParagraphIndex, long myElementIndex,
			long myCharIndex) {
		long location = 0;
	...
	public final List<ZLTextMark> getMarks() {
		return (myMarks != null) ? myMarks : Collections
				.<ZLTextMark> emptyList();
	}
	...
	public final ZLTextParagraph getParagraph(int index) {
		...
		return (kind == ZLTextParagraph.Kind.TEXT_PARAGRAPH) ? new ZLTextParagraphImpl(
				this, index)
