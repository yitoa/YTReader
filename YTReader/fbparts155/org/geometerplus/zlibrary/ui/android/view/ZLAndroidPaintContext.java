...
public final class ZLAndroidPaintContext extends ZLPaintContext {
	...
	private final Paint myGraphPaint = new Paint();
	...
	public ZLAndroidPaintContext(Canvas canvas, int width, int height,
			int scrollbarWidth) {
		myCanvas = canvas;
		...
		myOutlinePaint.setColor(Color.rgb(165, 165, 165));
		...
		if (!wallpaperFile.equals(ourWallpaperFile)) {
			...
			try {
				...
				case TILE_MIRROR: {
					...
					try {
						wallpaper = Bitmap.createBitmap(2 * w, 2 * h,
		...
		if (ourWallpaper != null) {
			...
			for (int cw = 0, iw = 1; cw < myWidth; cw += w, ++iw) {
				for (int ch = 0, ih = 1; ch < myHeight; ch += h, ++ih) {
					myCanvas.drawBitmap(ourWallpaper, cw, ch, myGraphPaint);
				}
			}
		} else {
	...
	public void clear(ZLColor color) {
		...
		myGraphPaint.setColor(ZLAndroidColorUtil.rgb(color));
	...
	public void fillPolygon(int[] xs, int ys[]) {
		...
		myCanvas.drawPath(path, myGraphPaint);
	}
	
	// 字体变更需要更新配置
	@Override
	public void setFont() {
		mySpaceWidth = -1;	//空格宽度（像素），需要重新获取
		myStringHeight = -1;//字符高度，运算行距要用
		myTextPaint.setTypeface(AndroidFontUtil.getYTTypeface());
		myTextPaint.setTextSize(AndroidFontUtil.getFontSize());
	}
	...
	@Override
	public void setFontInternal(String family, int size, boolean bold,
			boolean italic, boolean underline, boolean strikeThrought) {
		// myTextPaint.setTypeface(AndroidFontUtil.typeface(family, bold,
		// italic));
		myTextPaint.setTypeface(AndroidFontUtil.getYTTypeface());
		myTextPaint.setTextSize(AndroidFontUtil.getFontSize());
	...
	public void setFillColor(ZLColor color, int alpha) {
		myGraphPaint.setColor(ZLAndroidColorUtil.rgba(color, alpha));
	}
	...
	public int getStringWidth(char[] string, int offset, int length) {
		...
		if (!containsSoftHyphen) {
			String str = new String(string, offset, length);
			float len_f = myTextPaint.measureText(str) + 0.5f;
			return (int) len_f;
		} else {
	...
	public void drawImage(int x, int y, ZLImageData imageData, Size maxSize,
		...
		if (bitmap != null && !bitmap.isRecycled()) {
			myCanvas.drawBitmap(bitmap, x, y - bitmap.getHeight(), myGraphPaint);
		}
	}
	...
	public void fillRectangle(int x0, int y0, int x1, int y1) {
		...
		myCanvas.drawRect(x0, y0, x1 + 1, y1 + 1, myGraphPaint);
	}
}
