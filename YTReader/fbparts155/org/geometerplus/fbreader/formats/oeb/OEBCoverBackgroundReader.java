...
class OEBCoverBackgroundReader extends ZLXMLReaderAdapter implements XMLNamespaces {
	...
	public ZLFileImage readCover(ZLFile file) {
		...
		if(null == myImage) {
			try {
				myImage = imageByHref("images/cover.jpg");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
			...
			case READ_METADATA:
				if (testTag(OpenPackagingFormat, "meta", tag)) {
					...
					if ("cover".equals(name)) {
						myCoverId = attributes.getValue("content");// content值，<meta content="cover.jpg" name="cover" />
					}
				}
				break;
			case READ_MANIFEST:
			if ("item".equals(tag) && 
					(myCoverId.equals(attributes.getValue("id")) || attributes.getValue("id").equals("cover.jpg"))) {
				myImage = imageByHref(attributes.getValue("href"));
				if (myImage != null) {
					return true;
				}
