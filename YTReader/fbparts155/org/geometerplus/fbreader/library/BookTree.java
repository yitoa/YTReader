...
public class BookTree extends LibraryTree {
	public final FBBook Book;
	private final boolean myShowAuthors;

	BookTree(FBBook book, boolean showAuthors) {
		Book = book;
		myShowAuthors = showAuthors;
	}

	BookTree(LibraryTree parent, FBBook book, boolean showAuthors) {
		super(parent);
		Book = book;
		myShowAuthors = showAuthors;
	}

	BookTree(LibraryTree parent, FBBook book, boolean showAuthors, int position) {
	...
	@Override
	public boolean containsBook(FBBook book) {
	...
	public int compareTo(FBTree tree) {
		...
		if (cmp == 0 && tree instanceof BookTree) {
			final FBBook b = ((BookTree)tree).Book;
			if (Book != null && b != null) {
