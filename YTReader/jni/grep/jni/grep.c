...
//    fprintf(resultFP,"[out_file]%s%c", filename, sep);
  }
  if (out_line)
    {
      nlscan(beg);
      printf("%d%c", ++totalnl, sep);
//      fprintf(resultFP,"[out_line]%d%c", ++totalnl, sep);
      lastnl = lim;
    }
  if (out_byte){
    printf("%lu%c", totalcc + (beg - bufbeg), sep);
    fprintf(resultFP,"%lu%c\n", totalcc + (beg - bufbeg), sep);
	fprintf(resultFP, "%lu%c%lu\n", totalcc + (beg - bufbeg), sep,lim - beg);
  }
