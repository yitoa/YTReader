...
LVDocView::LVDocView(int bitsPerPixel) :
    ...
    //createDefaultDocument(cs16("No document"), lString16(L"Welcome to CoolReader! Please select file to open"));
...
void LVDocView::createDefaultDocument(lString16 title, lString16 message) {
	...
	_pos = 0;
	_page = 0;
	//开始创建dom树，文档尚未解析，测试用途，还分配那么多内存，坑爹啊
