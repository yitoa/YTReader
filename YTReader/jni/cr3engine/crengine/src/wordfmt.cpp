...
vPrologue1(diagram_type *pDiag, const char *szTask, const char *szFilename)
{
    ...
    //开始创建dom树，文档开始解析，根节点为元素1
    lString16 title("Word document");
    writer->OnTagOpen(NULL, L"?xml");				//不创建元素
    ...
    writer->OnTagOpenNoAttr(NULL, L"FictionBook");	//元素2
    // DESCRIPTION
    writer->OnTagOpenNoAttr(NULL, L"description");	//元素3
    writer->OnTagOpenNoAttr(NULL, L"title-info");	//元素4
    writer->OnTagOpenNoAttr(NULL, L"book-title");	//元素5
    writer->OnText(title.c_str(), title.length(), 0);//文本1
    writer->OnTagClose(NULL, L"book-title");
    writer->OnTagClose(NULL, L"title-info");
    writer->OnTagClose(NULL, L"description");
    // BODY
    writer->OnTagOpenNoAttr(NULL, L"body");			//元素6
} /* end of vPrologue1 */
    ...
    USHORT usFontSize, USHORT usMaxFontSize)
{
    ...
    if ( styleBold ) writer->OnTagOpenNoAttr(NULL, L"b");
    if ( styleItalic ) writer->OnTagOpenNoAttr(NULL, L"i");
    //=================
    writer->OnText(s.c_str(), s.length(), 0);
    //=================
    if ( styleItalic ) writer->OnTagClose(NULL, L"i");
    if ( styleBold ) writer->OnTagClose(NULL, L"b");
    if ( !style.empty() ) {
    	//CRLog::debug("</span>");
...
bool ImportWordDocument( LVStreamRef stream, ldomDocument * m_doc, LVDocViewCallback * progressCallback, CacheLoadingCallback * formatCallback )
{
    ...
    lUInt32 lFilesize = (lUInt32)stream->GetSize();//文件大小
    iWordVersion = iGuessVersionNumber(file, lFilesize);//word版本
    if (iWordVersion < 0 || iWordVersion == 3) {
        if (bIsRtfFile(file)) {
            CRLog::error("not a Word Document."
                " It is probably a Rich Text Format file");
