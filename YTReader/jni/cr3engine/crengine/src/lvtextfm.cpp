   ...
   lInt8           letter_spacing)
{
    ...
    if (flags & LTEXT_FLAG_OWNTEXT)
    {
        ...
        pline->t.text = (lChar16*)malloc( len * sizeof(lChar16) );//动态分配内存
        memcpy((void*)pline->t.text, text, len * sizeof(lChar16));//文本拷贝
    }
    else
    {
        pline->t.text = text;
    }
    pline->index = (lUInt16)(pbuffer->srctextlen-1);//行号
    //CRLog::debug("%d,%s", pline->index, U16toU8(pline->t.text, len));//添加文本内容，utf-16转utf-8
    pline->object = object;
    pline->t.len = (lUInt16)len;					//字符数
    pline->margin = margin;							//首行缩进
    pline->flags = flags;							//标志位
    pline->interval = interval;						//间隔
    pline->t.offset = offset;						//文本偏移位置
    pline->color = color;							//颜色
    pline->bgcolor = bgcolor;						//背景色
    pline->letter_spacing = letter_spacing;			//字符间距
}
...
public:
    ...
    int       m_length;	//本段字符数
    int       m_size;	//本段字符数+16保留空间
    bool      m_staticBufs;
    lChar16 * m_text;	//本段字符缓冲区，保存实际的文本内容
    lUInt8 *  m_flags;	//标志位
    src_text_fragment_t * * m_srcs;	//标记每个字符所在行
    lUInt16 * m_charindex;			//每个字在该行的偏移位置
    int *     m_widths;				//本行首字符到本字符的总宽度
    int m_y;						//临时变量，计算格式化行的y坐标
...
#define ITEMS_RESERVED 16
        ...
        memset( m_flags, 0, sizeof(lUInt8)*m_length );//每个字给一个标志位
        pos = 0;
    }
    ...
    void copyText( int start, int end )
    {
        ...
        int i;
        for ( i=start; i<end; i++ ) {
            src_text_fragment_t * src = &m_pbuffer->srctext[i];//第i行文本
            if ( src->flags & LTEXT_SRC_IS_OBJECT ) {//图片
                m_text[pos] = 0;
                ...
                int len = src->t.len;
                lStr_ncpy( m_text+pos, src->t.text, len );//拷贝一整行
                if ( i==0 || (src->flags & LTEXT_FLAG_NEWLINE) )
                    m_flags[pos] = LCHAR_MANDATORY_NEWLINE;//首行或者段落开头
                for ( int k=0; k<len; k++ ) {
                    m_charindex[pos] = k;
                    m_srcs[pos] = src;
...
//                    return; // no zoom
    ...
    void measureText()
    {
        ...
        int start = 0;
        int lastWidth = 0;
#define MAX_TEXT_CHUNK_SIZE 512		//一行最大字符数512*10px=5120px，足够了
        ...
        for ( i=0; i<=m_length; i++ ) {
            ...
            if ( i<m_length ) {
                newSrc = m_srcs[i];								//该字符所在行
                isObject = m_charindex[i] == OBJECT_CHAR_INDEX;	//该字符偏移位置是否为FFFF
            ...
            if ( i>start && (newFont!=lastFont || isObject || prevCharIsObject || i>=start+MAX_TEXT_CHUNK_SIZE || (m_flags[i]&LCHAR_MANDATORY_NEWLINE)) ) {
                ...
                if ( m_charindex[i-1]!=OBJECT_CHAR_INDEX ) {
                    ...
                    for ( int k=0; k<len; k++ ) {
                    	//从缓冲区拷贝进去。。。
                        m_widths[start + k] = lastWidth + widths[k];//累计从本段第一个字符到本字符的宽度
                    ...
                    lastWidth += widths[len-1];//加上缓冲区最后一个字符的值
                } else {
                    // measure object
                    // assume i==start+1
...
#define MAX_WORD_SIZE 64
    ...
    void alignLine( formatted_line_t * frmline, int width, int alignment ) {
        if ( frmline->x + frmline->width > width ) {//太宽，降低空格宽度，以适应行宽
            // line is too wide
...
//                int addSpaceMod = extraSpace % addSpacePoints;
    ...
    /// split line into words, add space for width alignment，切分一行文本为一个个的单词
    //start：起始字符
    //end：结束字符
    //x：起始x坐标
    void addLine( int start, int end, int x, src_text_fragment_t * para, int interval, bool first, bool last, bool preFormattedOnly, bool needReduceSpace )
    {
        ...
        formatted_line_t * frmline =  lvtextAddFormattedLine( m_pbuffer );//分配格式化行空间
        frmline->y = m_y;	//行的y坐标
        frmline->x = x;		//行的x起始x坐标
        src_text_fragment_t * lastSrc = m_srcs[start];//本行原始数据，要重新格式化
        //CRLog::debug("s=%d,e=%d,x=%d,行号=%d,txt=%s", start, end, x, m_pbuffer->frmlinecount - 1, U16toU8(m_text +start, end - start));
        int wstart = start;
        ...
        for ( int i=start; i<=end; i++ ) {
            ...
            if ( i>wstart && (newSrc!=lastSrc || space || lastWord) ) {
                ...
                formatted_word_t * word = lvtextAddFormattedWord(frmline);//分配单词空间
                int b;
                int h;
                word->src_text_index = m_srcs[wstart]->index;//字所在的行号
...
//                        wy = ((font->getSize() * (16-interval)) >> 4) >> 1;
                    ...
                    word->x = frmline->width;	//行内的x坐标（相对）
                    word->flags = 0;			//单词的标志位
                    word->t.start = m_charindex[wstart];//行号
                    word->t.len = i - wstart;			//单词由len个字符组成
                    word->width = m_widths[i>0 ? i-1 : 0] - (wstart>0 ? m_widths[wstart-1] : 0);//单词的宽度（像素）
                    word->min_width = word->width;
...
//                        CRLog::trace("last char is UNICODE_NO_BREAK_SPACE");
                    ...
                    if ( m_flags[i-1] & LCHAR_IS_SPACE) {//字符为空格
...
//                        else
                ...
                if ( frmline->height < frmline->baseline + h )
                    frmline->height = (lUInt16) ( frmline->baseline + h );//字体像素 + 行距

                frmline->width += word->width;//行宽
        ...
        //CRLog::debug("y=%d,行宽=%d,本段字符数=%d,s=%d,e=%d,x=%d,行号=%d,txt=%s", frmline->y, frmline->width, m_length, start, end, x, m_pbuffer->frmlinecount - 1, U16toU8(m_text +start, end - start));
        m_y += frmline->height;
    ...
    void processParagraph( int start, int end )
    {
        ...
        // split paragraph into lines, export lines，将整段的内容拆分为一行一行数据
        int pos = 0;
        int indent = m_srcs[0]->margin;
        //CRLog::debug("%s", U16toU8(para->t.text, para->t.len));
        for (;pos<m_length;) {
...
void LFormattedText::Draw( LVDrawBuf * buf, int x, int y, ldomMarkedRangeList * marks, ldomMarkedRangeList *bookmarks )
{
    ...
    for (i=0; i<m_pbuffer->frmlinecount; i++)
    {
        ...
        if (line_y + frmline->height>=clip.top)
        {
            ...
            {
                word = &frmline->words[j];
                if (word->flags & LTEXT_WORD_IS_OBJECT)//图片
                {
                    srcline = &m_pbuffer->srctext[word->src_text_index];
