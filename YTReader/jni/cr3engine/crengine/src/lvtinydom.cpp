...
{
	lUInt32 shift = TNC_PART_INDEX_SHIFT;
	lUInt32 mask = TNC_PART_MASK;
	lUInt32 index_shift = index>>shift;
	lUInt32 index_shift4 = index>>4;
    if ( !index )
        return NULL;
    if ( index & 1 ) // element
        return &(_elemList[index_shift][index_shift4&mask]);
    else // text
        return &(_textList[index_shift][index_shift4&mask]);
}
...
ldomNode * tinyNodeCollection::allocTinyNode( int type )
{
    ...
    if ( type & 1 ) {
        ...
        if ( _elemNextFree ) {
            ...
            if ( !part ) {
            	int dom_mem_size = sizeof(ldomNode);
                part = (ldomNode*)malloc( dom_mem_size * TNC_PART_LEN );//创建1024个节点
                memset( part, 0, dom_mem_size * TNC_PART_LEN );
...
int ldomTextStorageChunk::addText( lUInt32 dataIndex, lUInt32 parentIndex, const lString8 & text )
{
    ...
    if ( !_buf ) {
        ...
        _buf = (lUInt8*)malloc(sizeof(lUInt8) * _bufsize);//如果缓冲区为空，则分配大块空间
...
int ldomDocument::render( LVRendPageList * pages, LVDocViewCallback * callback, int width, int dy, bool showCover, int y0, font_ref_t def_font, int def_interline_space, CRPropRef props )
{
    ...
    if ( !_rendered ) {
        ...
        LVRendPageContext context( pages, _page_height );//渲染页上下文
        ...
        //updateStyles();
        CRLog::trace("rendering...");//文档解析完毕，开始渲染
        int height = renderBlockElement( context, getRootNode(), 0, y0, width ) + y0;//视图总高度
        _rendered = true;
//#define _DEBUG
    #ifdef _DEBUG
        LVStreamRef ostream = LVOpenFileStream( "/mnt/sdcard/test.xml", LVOM_WRITE );
        saveToStream( ostream, "utf-16" );
        CRLog::error("test_save_after_init_rend_method");
    #endif
        gc();
...
bool ldomXPointer::getRect(lvRect & rect) const
{
    ...
    if ( finalNode!=NULL ) {
        ...
        int lastLen = -1;
        int lastOffset = -1;
        ldomXPointerEx xp(node, offset);//获取当前元素的dom指针
        int srcCount = txtform->GetSrcCount();
        for ( int i=0; i < srcCount; i++ ) {
            const src_text_fragment_t * src = txtform->GetSrcInfo(i);//第i行原始文本
            ...
            lastIndex = i;
            lastLen =  isObject ? 0 : src->t.len;		//该行字符数
            lastOffset = isObject ? 0 : src->t.offset;	//该行偏移地址
            ldomXPointerEx xp2((ldomNode*)src->object, lastOffset);//获取第i行原始文本的dom指针
            if ( xp2.compare(xp)>0 ) {
                srcIndex = i;
                srcLen = lastLen;
                offset = lastOffset;
        ...
        int lineCount = txtform->GetLineCount();//本段格式化行数
        for ( int l = 0; l < lineCount; l++ ) {
            const formatted_line_t * frmline = txtform->GetLineInfo(l);//第l行格式化行
            int word_count = (int)frmline->word_count;
            for ( int w=0; w < word_count; w++ ) {
                const formatted_word_t * word = &frmline->words[w];
                bool lastWord = (l == lineCount - 1 && w == word_count - 1);
...
void ldomXPointerEx::initIndex()
{
    ...
    _level = 0;
    while ( p ) {
        m[_level] = p->getNodeIndex();//当前节点号
        _level++;
        p = p->getParentNode();//父节点
    }
...
bool ldomDocument::findText( lString16 pattern, bool caseInsensitive, bool reverse, int minY, int maxY, LVArray<ldomWord> & words, int maxCount, int maxHeight )
{
    ...
    if ( maxY<=0 || maxY>fh )
        maxY = fh;
    ldomXPointer start = createXPointer( lvPoint(0, minY), reverse?-1:1 );	//根据起始坐标，获取起始段落点
    ldomXPointer end = createXPointer( lvPoint(10000, maxY), reverse?-1:1 );//根据结束坐标，获取结束段落点
...
lString16 ldomDocument::getSearchTitle() {
	str_search = str_search_1;
	return str_search;
}
...
{
	//CRLog::debug("pos = %d, txt=%s", pos, LCSTR(str));
    int len = pattern.length();
...
bool ldomXRange::findText( lString16 pattern, bool caseInsensitive, bool reverse, LVArray<ldomWord> & words, int maxCount, int maxHeight, bool checkMaxFromStart )
{
    ...
    if ( reverse ) {
        ...
        // direct search
        if ( !_start.isText() ) {
        	//CRLog::info("不是文本，指向下一组可视文本");
            _start.nextVisibleText();
        }
        int firstFoundTextY = -1;
        if (checkMaxFromStart) {
			ldomXPointer p( _start.getNode(), _start.getOffset() );
			firstFoundTextY = p.toPoint().y;//起始高度
			//CRLog::debug("startY = %d", firstFoundTextY);
		}
        while ( !isNull() ) {
            ...
            if ( firstFoundTextY!=-1 && maxHeight>0 ) {
				...
				//CRLog::info("currentY=%d < %d", currentTextY, firstFoundTextY + maxHeight);
            }
            ...
            while ( ::findText( txt, offs, pattern ) ) {
                if ( !words.length() && maxHeight>0 ) {
                    ldomXPointer p( _start.getNode(), offs );
                    int currentTextY = p.toPoint().y;
                    if(currentTextY < firstFoundTextY) {
                    	CRLog::error("currentY=%d < %d=firstY", currentTextY, firstFoundTextY);
                    }
                    if (checkMaxFromStart) {
						if ( currentTextY >= firstFoundTextY+maxHeight ) {
							//CRLog::info("currentY >= firstFoundTextY+maxHeight退出");
							return words.length()>0;
						}
					} else
				...
				if (0 == str_search_1.length()) {
					...
					str_search_1 = (len_txt - offs <= 50) ? txt.substr(offs) : txt.substr(offs, 50);
				}
                words.add( ldomWord(_start.getNode(), offs, offs + pattern.length() ) );//找到，添加到LVArray<ldomWord>中
                offs++;
            }
            if ( !_start.nextVisibleText() ){
            	//CRLog::info("正向搜索：!_start.nextVisibleText()");
                break;
            }
            if ( words.length() >= maxCount ){
            	//CRLog::info("正向搜索：words.length() >= maxCount");
                break;
            }
        }
    }
...
bool ldomDocument::loadCacheFileContent(CacheLoadingCallback * formatCallback)
{
    ...
    CRLog::trace("ldomDocument::loadCacheFileContent()");
    {
        ...
        _hdr = h;
        //CRLog::info("Loaded render properties: styleHash=%x, stylesheetHash=%x, docflags=%x, width=%x, height=%x", _hdr.render_style_hash, _hdr.stylesheet_hash, _hdr.render_docflags, _hdr.render_dx, _hdr.render_dy);
    }
...
ContinuousOperationResult ldomDocument::saveChanges( CRTimerUtil & maxTime )
{
    ...
    case 9:
        ...
        //CRLog::info("Saving render properties: styleHash=%x, stylesheetHash=%x, docflags=%x, width=%x, height=%x",_hdr.render_style_hash, _hdr.stylesheet_hash, _hdr.render_docflags, _hdr.render_dx, _hdr.render_dy);
...
void ldomDocument::updateRenderContext()
{
    ...
    //CRLog::info("Updating render properties: styleHash=%x, stylesheetHash=%x, docflags=%x, width=%x, height=%x", _hdr.render_style_hash, _hdr.stylesheet_hash, _hdr.render_docflags, _hdr.render_dx, _hdr.render_dy);
}
...
{
    ASSERT_NODE_NOT_NULL;
    //CRLog::trace("ldomNode:%s", s8.c_str());
    if  ( isElement() ) {
        if ( isPersistent() )
            modify();
        tinyElement * me = NPELEM;
...
int ldomNode::renderFinalBlock(  LFormattedTextRef & frmtext, RenderRectAccessor * fmt, int width )
{
    ...
    f = getDocument()->createFormattedText();//创建下一段的格式化文本对象
    if ( (rm != erm_final && rm != erm_list_item && rm != erm_table_caption) )
        return 0;
    //RenderRectAccessor fmt( this );
    /// render whole node content as single formatted object
    int flags = styleToTextFmtFlags( getStyle(), 0 );
    ::renderFinalBlock( this, f.get(), fmt, flags, 0, 16 );//获取本段原始数据
    int page_h = getDocument()->getPageHeight();
    cache.set( this, f );
    int h = f->Format((lUInt16)width, (lUInt16)page_h);		//本段原始数据格式化
    frmtext = f;
