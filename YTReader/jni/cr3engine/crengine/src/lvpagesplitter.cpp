...
{
    renderedFinalBlocks += numFinalBlocksRendered;//总数
...
public:
    ...
    const LVRendLineInfo * pagestart;	//指向首行
    const LVRendLineInfo * pageend;		//指向末行
    const LVRendLineInfo * next;		//指向下一行
    const LVRendLineInfo * last;		//指向上一行
    int   footheight;
    LVFootNote * footnote;
...
void LVRendPageContext::split()
{
    if ( !page_list )
        return;
    PageSplitState s(page_list, page_h);//根据page_h把格式化行分页，保存在page_list

    int lineCount = lines.length();//格式化行总数
