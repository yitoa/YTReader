...
#define TNC_PART_INDEX_SHIFT (TNC_PART_SHIFT+4) //14
#define TNC_PART_LEN (1<<TNC_PART_SHIFT)		//1024
#define TNC_PART_MASK (TNC_PART_LEN-1)			//1023(0x3FF)
/// storage of ldomNode
class tinyNodeCollection
{
...
private:
    int _textCount;//文本节点数
    lUInt32 _textNextFree;
    ldomNode * _textList[TNC_PART_COUNT];//dom文本节点,二维的，每个节点包含1024个字节点
    int _elemCount;//元素节点数
    lUInt32 _elemNextFree;
    ldomNode * _elemList[TNC_PART_COUNT];//dom元素节点,二维的，每个节点包含1024个字节点
    LVIndexedRefCache<css_style_ref_t> _styles;
    LVIndexedRefCache<font_ref_t> _fonts;
    int _tinyElementCount;
    int _itemCount;// = _textCount + _elemCount
    int _docIndex;

protected:
#if BUILD_LITE!=1
    /// final block cache
    CVRendBlockCache _renderedBlockCache;//格式化文本，段落
    CacheFile * _cacheFile;
    bool _mapped;
    bool _maperror;
    int  _mapSavingStage;
...
{
protected:
    int _indexes[MAX_DOM_LEVEL];//dom位置
    int _level;					//dom第level层
    void initIndex();
public:
...
public:
    ...
    bool findText( lString16 pattern, bool caseInsensitive, bool reverse, LVArray<ldomWord> & words, int maxCount, int maxHeight, bool checkMaxFromStart = true );
};
...
public:
    lString16 str_search;
    lString16 getSearchTitle();
private:
    LVTocItem m_toc;
#if BUILD_LITE!=1
