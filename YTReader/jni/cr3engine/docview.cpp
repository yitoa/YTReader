...
bool DocViewNative::findText( lString16 pattern, int origin, bool reverse, bool caseInsensitive )
{
    ...
    //CRLog::debug("CRViewDialog::findText: top=%d, bottom=%d, h=%d", rc.top, rc.bottom, rc.height());
    //CRLog::debug("CRViewDialog::findText: Current page: %d .. %d", rc.top, rc.bottom);
    //CRLog::debug("CRViewDialog::findText: searching '%s' from %d to %d origin=%d pageHeight=%d", LCSTR(pattern), start, end, origin, pageHeight );
    _docview->clearSelection();
 ...
 * Signature: ()V
 */
JNIEXPORT jstring JNICALL Java_org_coolreader_crengine_DocView_getSearchTitle
  (JNIEnv * _env, jobject _this)
{
    CRJNIEnv env(_env);
    DocViewNative * p = getNative(_env, _this);
    return env.toJavaString(p->_docview->getDocument()->getSearchTitle());
}
